...
def get_rays_torch(H, W, K, R, T):
    ...
    #rays_o = -np.dot(R.T, T).ravel()
    rays_o = -torch.mm(R.T, T).ravel()

    # calculate the world coodinates of pixels
    #i, j = np.meshgrid(np.arange(W, dtype=np.float32),
                       #np.arange(H, dtype=np.float32),
    #                   indexing='xy')
    i, j = torch.meshgrid(torch.arange(W, dtype=np.float32), 
                          torch.arange(W, dtype=np.float32),
                          indexing="xy")
    ...
    pixel_camera = torch.mm(xy1, torch.linalg.inv(K).T)
    pixel_world = torch.mm(pixel_camera - T.ravel(), R)
...
def get_bound_corners_torch(bounds):
    min_x, min_y, min_z = bounds[0]
    max_x, max_y, max_z = bounds[1]
...
def get_bound_2d_mask_torch(bounds, K, pose, H, W):
    device = K.device
    corners_3d = get_bound_corners_torch(bounds)
    corners_2d = base_utils.project_torch(corners_3d, K, pose)
    corners_2d = torch.round(corners_2d).to(torch.int32) #.astype(int)
    #mask = np.zeros((H, W), dtype=np.uint8)
...
def sample_ray_torch(img, msk, K, R, T, bounds, nrays, split, opt):
    H, W = img.shape[:2]
    
    ray_o, ray_d = get_rays_torch(H, W, K, R, T)

    device = img.device

    pose = torch.cat([R, T], axis=1)
    bound_mask = get_bound_2d_mask_torch(bounds, K, pose, H, W)
...
def sample_ray(img, msk, K, R, T, bounds, nrays, split, opt, is_random = False):
    ...
    if (not is_random) and (split=="evaluate" or (split == 'train' and (opt.sample_all_pixels or (opt.vrnr)))):
