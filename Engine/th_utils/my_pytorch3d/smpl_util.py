...
from pytorch3d.structures import Meshes
def mesh_normal(vertices, faces):
    ...
    mesh = Meshes(verts = vertices, faces = faces,
                        textures=None).cuda() #[[i], ...]
    return mesh.verts_normals_packed().reshape(batch_size, -1, 3)
...
class SMPL_Util:
    ...
    def get_normal(self, smpl_vts):
        if smpl_vts.ndimension() < 3:
            smpl_vts = smpl_vts[None,...]
        batch_size = smpl_vts.shape[0]
        
        norm = mesh_normal(smpl_vts, self.smpl_faces).reshape(smpl_vts.shape)
        
        if norm.ndimension() < smpl_vts.ndimension():
            return norm[None,...]
        return norm
