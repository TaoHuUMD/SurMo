...
from .torch_engine.ops import upfirdn2d
from .torch_utils import persistence
from .torch_utils import misc
  
from .networks_stylegan2 import SynthesisBlock, SynthesisBlock_Rec
import numpy as np
...
class SuperresolutionHybrid8X(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
        ...
        if type(img_resolution) == tuple:
            img_resolution = max(img_resolution)
...
class SuperresolutionHybrid8XDC(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
        ...
        if type(img_resolution) == tuple or type(img_resolution) == list:
            img_resolution = max(img_resolution)
...
# for 256x512 generation, rectangle
@persistence.persistent_class
class SuperresolutionHybrid8XDC_Rectangle(torch.nn.Module):
    def __init__(self, channels, input_resolution, final_resolution, w_dim, sr_num_fp16_res, sr_antialias,
                num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None,# IGNORE
                **block_kwargs):
        super().__init__()
    ...
    def forward(self, rgb, x, style2d, ws, **block_kwargs):
        ws = ws[:, -1:, :].repeat(1, 3, 1)

        assert x.shape[-1] == self.input_resolution
        # if x.shape[-1] != self.input_resolution[-1]:
        #     x = torch.nn.functional.interpolate(x, size=(self.input_resolution[0], self.input_resolution[1]),
        #                                           mode='bilinear', align_corners=False, antialias=self.sr_antialias)
        #     rgb = torch.nn.functional.interpolate(rgb, size=(self.input_resolution[0], self.input_resolution[1]),
        #                                           mode='bilinear', align_corners=False, antialias=self.sr_antialias)
...
# for 256x512 generation, rectangle
@persistence.persistent_class
class StyleSuperresolution_Rectangle(torch.nn.Module):
    def __init__(self, channels, input_resolution, final_resolution, w_dim, sr_num_fp16_res, sr_antialias,
                num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None, phase = "train",# IGNORE
                **block_kwargs):
        super().__init__()
        ...
        if isinstance(input_resolution, int):
            ...
            final_resolution = (final_resolution, final_resolution)

        self.final_resolution = final_resolution
        self.input_resolution = input_resolution
        self.sr_antialias = sr_antialias

        self.scale_factor = int(final_resolution[0] / input_resolution[0])

        self.upnet = []

        fused_modconv_default = False if phase == "train" else True

        self.up1 = SynthesisBlock_Rec(channels, 256, w_dim=w_dim, resolution = tuple([2*x for x in self.input_resolution]),
            img_channels=3, is_last=False, conv_clamp = None, fused_modconv_default = fused_modconv_default, **block_kwargs)            
        self.up2 = SynthesisBlock_Rec(256, 128, w_dim=w_dim, resolution = tuple([self.scale_factor * x for x in self.input_resolution]),
            img_channels=3, is_last=True,  conv_clamp=None, fused_modconv_default = fused_modconv_default, **block_kwargs)
    ...
    def forward(self, rgb, x, style2d, ws, **block_kwargs):
        ...
        assert x.shape[-1] == self.input_resolution[-1]
        # if x.shape[-1] != self.input_resolution[-1]:
        #     x = torch.nn.functional.interpolate(x, size=(self.input_resolution[0], self.input_resolution[1]),
        #                                           mode='bilinear', align_corners=False, antialias=self.sr_antialias)
        #     rgb = torch.nn.functional.interpolate(rgb, size=(self.input_resolution[0], self.input_resolution[1]),
        ...
        latent = style2d if style2d is not None else ws

        if self.scale_factor == 2:
            tx, trgb = self.up2(x, rgb, latent, **block_kwargs)
            return trgb

        tx, trgb = self.up1(x, rgb, latent, **block_kwargs)
        tx, trgb = self.up2(tx, trgb, latent, **block_kwargs)
        return trgb
...
class UpsampleNet(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        self.up1 = Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, up=2,
            resample_filter=resample_filter, channels_last=self.channels_last)

        self.up2 = Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, up=2,
            resample_filter=resample_filter, channels_last=self.channels_last)
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, **layer_kwargs):
        ...
        if img is not None:
            ...
            img = upfirdn2d.upsample2d(img, self.resample_filter)
        if self.is_last or self.architecture == 'skip':
            #print("call rgb", self.num_torgb, len(ws.unbind(dim=1)))
            y = self.torgb(x, next(w_iter), fused_modconv=fused_modconv)
            #y = x[:,:3,...]
            y = y.to(dtype=torch.float32, memory_format=torch.contiguous_format)
            img = img.add_(y) if img is not None else y
