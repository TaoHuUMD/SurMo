...
#----------------------------------------------------------------------------
@persistence.persistent_class
class SingleDiscriminatorHuman(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
        ...
        self.img_resolution = img_resolution
        max_reso = max(img_resolution)

        h_ratio, w_ratio = img_resolution[0] / max_reso, img_resolution[1] / max_reso 


        self.img_resolution = img_resolution
        self.img_resolution_log2 = int(np.log2(max_reso))
        self.img_channels = img_channels
        self.block_resolutions = [2 ** i for i in range(self.img_resolution_log2, 2, -1)]
        channels_dict = {res: min(channel_base // res, channel_max) for res in self.block_resolutions + [4]}
        fp16_resolution = max(2 ** (self.img_resolution_log2 + 1 - num_fp16_res), 8)
        ...
        cur_layer_idx = 0
        for res in self.block_resolutions:
            in_channels = channels_dict[res] if res < max_reso else 0
            tmp_channels = channels_dict[res]
            out_channels = channels_dict[res // 2]
            #use_fp16 = (res >= fp16_resolution)
            block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=(int(h_ratio*res), int(w_ratio*res)),
                first_layer_idx=cur_layer_idx, use_fp16=use_fp16, **block_kwargs, **common_kwargs)
            setattr(self, f'b{res}', block)
            cur_layer_idx += block.num_layers
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=tuple([int(4*r) for r in [h_ratio, w_ratio]]), **epilogue_kwargs, **common_kwargs)
...
@persistence.persistent_class
class DualDiscriminatorHuman(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
