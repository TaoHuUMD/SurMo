...
class FashionDiscConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True, activate=False):
        super(FashionDiscConv2d, self).__init__()
...
class FashionResBlock(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        if out_channel != in_channel:
            self.skip = FashionDiscConv2d(in_channel, out_channel, 1)
        else:
            self.skip = None
...
class FashionDiscriminator(nn.Module):
    def __init__(self, input_dim, init_size):
        super().__init__()
        ...
        convs = [FashionDiscConv2d(self.input_dim, channels[self.init_size], 1, activate=True)]
        ...
        for i in range(log_size-1, 0, -1):
            ...
            convs.append(FashionResBlock(in_channel, out_channel))

            in_channel = out_channel

        self.convs = nn.Sequential(*convs)

        # self.final_conv = FashionDiscConv2d(in_channel, final_out_channel, 2)
    ...
    def forward(self, input):
        ...
        os = out.reshape(-1, self.in_channel * self.multiplier)
        out1 = self.final_conv(os)
        # out = out.mean(-2)
        gan_preds = out1[:,0:1]
