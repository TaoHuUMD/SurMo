...
def merge_config_opt(config_file, opt):
...
def yaml_config(config, default_cofig, resume_latest=False, num_workers=1):
    ...
    data_config = config
    conf = edict(yaml.load(open(data_config), Loader=yaml.SafeLoader))

    def copy(conf, default):
        for key in conf:
            if key in default and isinstance(default[key], edict):
...
class BaseOptions():
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--save_tmp_rendering', action='store_true')
        self.parser.add_argument('--load_tmp_rendering', action='store_true')
        self.parser.add_argument('--tmp_train', action='store_true')
        ...
        # for setting inputs
        self.parser.add_argument('--dataset', type=str, default='zju_mocap')
        ...
        self.parser.add_argument('--face_model', type=str, default='./data/asset/spretrains/sphere20a_20171020.pth')
    ...
    def set_methods(self):
        ...
        self.parser.add_argument('--org_NB', action='store_true', help="anr compare")
    ...
    def removed_config(self, opt):
        method_config = []
        method_config.append("motion.yml")
    ...
    def merge_cfg(self, opt):
        ...
        dataset_base_cfg = os.path.join(opt.config_dir, "configs/datasets/%s/base.yml" % opt.config[0].split("/")[0])
    ...
    def parse(self, save=True):
        ...
        if self.isTrain:
            ...
            if len(self.opt.multiview_ids) ==0:
                print("no task on this thread, exit")
                exit()   
