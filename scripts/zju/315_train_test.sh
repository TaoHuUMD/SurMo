...
config="--config zju/motion_315_fv.yml --project_config uvm.yml --method_config motion.yml"
...
#training script
batch_motion "${gpu}" ${method} "mtrain" "" "${modelname}" "${config} ${name}  ${cmd}" "-1"
exit